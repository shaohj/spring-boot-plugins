...
public class ExcelUtils {
	...
	public static void export(String tempFileName, Object context, OutputStream out) throws ExcelException {
		try {
			//打开工作簿 并进行初始化
			XSSFWorkbook inWb = WorkbookUtils.openWorkbookByProPath(tempFileName);
	...
	public static void parseWorkbook(Object context, XSSFWorkbook inWb, SXSSFWorkbook outWb) throws ExcelException {
		try {
			...
			for (int inSheetIndex = 0; inSheetIndex < inSheetCount; inSheetIndex++) {
				...
				//设置输出sheet的列宽和模板一致
				int maxCellNum = SheetUtils.getMaxCellNum(inSheet);
				int[] cellWidths = SheetUtils.getCellWidths(inSheet, maxCellNum);
				for (int i = 0; i < maxCellNum; i++) {
					outSheet.setColumnWidth(i, cellWidths[i]);
				}

				//根据模板边读边写导出数据
				ExcelParser.parse(context, outWb, inSheet, outSheet, inSheet.getFirstRowNum(), inSheet.getLastRowNum());
			}
		} catch (Exception e) {
			if(e instanceof ExcelException){
				throw (ExcelException)e;
