package com.poi.template.excel.parse.cache;

import java.lang.reflect.Array;
import java.util.*;

import com.poi.template.excel.parse.cache.tags.ICacheTag;
...
import com.poi.template.excel.utils.PoiCommonUtils;
...
public class CacheExcelParser {
	...
	static {
		tagPackageMap.put(ICacheTag.class.getPackage().getName(), ICacheTag.class.getPackage().getName());
	}
	...
	public static void parse(Object context, SXSSFWorkbook outWb, Sheet inSheet, Sheet outSheet, int inSheetStartRow, int inSheetEndRow) {
		...
		while (readRowNum <= inSheetEndRow) {
			...
			// 设置当前导出的数据在第几行
			CacheExcelUtils.addValue(context, "currentRowNo", new Integer(writeRowNum + 1));
			...
			for (short colnum = readRow.getFirstCellNum(); colnum <= readRow.getLastCellNum(); colnum++) {
				...
				//复制样式和值
				CacheWorkbookUtils.copyCell(outWb, inCell, outCell);
				...
				ICacheTag tag = getTagClass(cellstr); //判断该表达式是否为标签
				if (null != tag) {
					//读取位置动态处理
	...
	public static ICacheTag getTagClass(String str) {
		String tagName = "";
		...
		tagName = "Cache" + tagName.substring(0, 1).toUpperCase() + tagName.substring(1, tagName.length());
		tagName += "Tag";

		// find in tagMap first, if not exist, search in the package
		ICacheTag tag = (ICacheTag) tagMap.get(tagName);
		if (tag == null) {
			...
			while (tagPackages.hasNext() && null == tag) {
				...
				try {
					...
					tag = (ICacheTag) clazz.newInstance();
				} catch (Exception e) {
					tag = null;
				}
			}
	...
	public static void parseCell(Object context, Sheet fromSheet, Sheet toSheet, Row row, Cell readCell, Cell writeCell) {
		...
		if (row.getRowNum() - 1 >= fromSheet.getFirstRowNum() && bMerge) {
			Row lastRow = CacheWorkbookUtils.getRow(row.getRowNum() - 1, fromSheet);
			Cell lastCell = CacheWorkbookUtils.getCell(lastRow, readCell.getColumnIndex());
