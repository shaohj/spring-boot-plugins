...
/**
 * 每合并一次文件,都会保存合并文件一次,再打开。优点:处理大文件可能有用,缺点:更大消耗
 * POI有bug,多次关闭,打开文件,关闭无效,打开文件失败.poi workbook close无效
 * @author ShaoHj
 *
 */
public class MergeExcelUtilByMergeFileOnce {
	
	private static Logger logger = LoggerFactory.getLogger(MergeExcelUtilByMergeFileOnce.class);
		...
		if(file.isDirectory()){ //dir为一个目录才会继续进行合并操作
			...
			if(CollectionUtils.isEmpty(excelFiles)){
				...
				if(excelFiles.size() == 1){
					...
					/* 第1个文件直接复制到合并文件,修改下sheet名称,防止sheet名称重复 */
					FileUtils.copyFile(dir + File.separator + excelFiles.get(0), mergeFilePath);
					
					Workbook mergeWb = null;
					try{
						mergeWb = WorkbookUtils.openWorkbookByAbsolute(mergeFilePath);
					...
					for(int i=1; i<excelFiles.size(); i++){
						copyExcel(dir + File.separator + excelFiles.get(i), mergeFilePath);
					}
				}
			}
		} else {
	 ...
	 * @throws IOException 
	 */
	public static void copyExcel(String fromFilePath, String toFilePath) throws ExcelException, IOException{
		Workbook toWb = null;
		Workbook formWb = null;
		try{
			toWb = WorkbookUtils.openWorkbookByAbsolute(toFilePath);
			...
			while(itor.hasNext()){
				...
				MergeExcelUtilByMergeFileOnce.copySheet(formWb, toWb, fromSheet, toSheet);
			}
			
			POIUtils.writeFile(toWb, toFilePath);
		} finally {
			IOUtils.close(toWb);
